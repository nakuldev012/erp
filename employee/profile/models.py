...
class PersonalEmpInfo(AbstractTime):
    ...
    emergency_contact_mobile_number = models.CharField(
        validators=[phone_validator], max_length=15
    )
...
class AccountEmpInfo(AbstractTime):
    emp_id = models.OneToOneField(
        ...
        related_name="accountempinfo_primempinfo",
    )
    bank_name = models.CharField(max_length=500, null=True, blank=True)
    aadhar_no = models.CharField(
        max_length=12, validators=[MinLengthValidator(10)], unique=True
    )
...
class AddressEmpInfo(BaseAddress):
    emp = models.ForeignKey(
        PrimaryEmpInfo,
        on_delete=models.CASCADE,
        related_name="addressempinfo_primempinfo",
    )
    address_proof = models.ForeignKey(
        UploadedFile, on_delete=models.CASCADE, related_name="addressempinfo_uploadfile"
    )
    isVerified = models.BooleanField("Verified", default=False)
    is_both_address_same = models.BooleanField("Is Both Address Same", default=False)

    def __str__(self):
        return f"{self.emp.user_id.email}_{self.type}_address"
...
class AcademicEmpInfo(AbstractTime):
    employee = models.ForeignKey(
        PrimaryEmpInfo,
        on_delete=models.CASCADE,
        related_name="academicempinfo_primempinfo",
    )
    board_or_university = models.ForeignKey(
        MasterConfig,
        on_delete=models.CASCADE,
        related_name="academicempinfo_masterconfig_boardoruniversity",
    )
    qualification_type = models.ForeignKey(
        MasterConfig,
        on_delete=models.CASCADE,
        related_name="academicempinfo_masterconfig_qualification_type",
    )
    institute_name = models.CharField(max_length=250)
    city = models.ForeignKey(
        City,
        on_delete=models.CASCADE,
        related_name="academicempinfo_city",
        null=True,
        blank=True,
    )
    ...
    mark_sheet = models.ForeignKey(
        UploadedFile,
        on_delete=models.CASCADE,
        related_name="academicempinfo_uploadfile_marksheet",
    )
    passing_certificate = models.ForeignKey(
        UploadedFile,
        on_delete=models.CASCADE,
        related_name="academicempinfo_uploadfile_passingcertificate",
    )
    is_highest_qualification = models.BooleanField(default=False)
...
class CertificationEmpInfo(AbstractTime):
    employee = models.ForeignKey(
        PrimaryEmpInfo,
        on_delete=models.CASCADE,
        related_name="certificationempinfo_primempinfo",
    )
...
class DoctorateEmpInfo(AbstractTime):
    employee = models.ForeignKey(
        PrimaryEmpInfo, on_delete=models.CASCADE, related_name="doctorate_primempinfo"
    )
    phd_status = models.ForeignKey(
        MasterConfig,
        on_delete=models.CASCADE,
        related_name="doctorate_masterconfig_phdstatus",
    )
    university = models.ForeignKey(
        MasterConfig,
        on_delete=models.CASCADE,
        related_name="doctorate_masterconfig_university",
    )
    ...
    degree = models.ForeignKey(
        UploadedFile,
        on_delete=models.CASCADE,
        related_name="doctorate_upload_degree",
        null=True,
        blank=True,
    )
    is_verified = models.BooleanField("Verified", default=False)
...
class EmpPreviousExp(AbstractTime):
    employee = models.ForeignKey(
        PrimaryEmpInfo, on_delete=models.CASCADE, related_name="prev_exp_primempinfo"
    )
    experience_type = models.ForeignKey(
        MasterConfig,
        on_delete=models.CASCADE,
        related_name="expierence_master_exp_type",
    )
    ...
    agp = models.CharField(
        max_length=100,
        null=True,
        blank=True,
    )
    gross_slary = models.CharField(
        max_length=100,
        null=True,
        blank=True,
    )
    ...
    experience_letter = models.ForeignKey(
        UploadedFile,
        on_delete=models.CASCADE,
        related_name="prevexp_upload_exp_letter",
        null=True,
        blank=True,
    )
    relieving_letter = models.ForeignKey(
        UploadedFile,
        on_delete=models.CASCADE,
        related_name="prevexp_upload_relieving_letter",
        null=True,
        blank=True,
    )
    is_verified = models.BooleanField("Verified", default=False)
...
class EmpCurrentExp(AbstractTime):
    employee = models.ForeignKey(
        PrimaryEmpInfo, on_delete=models.CASCADE, related_name="current_exp_primempinfo"
    )
    experience_type = models.ForeignKey(
...
class EmpReporting(AbstractTime):
    employee = models.ForeignKey(
        PrimaryEmpInfo,
        on_delete=models.CASCADE,
        related_name="reporting_primempinfo",
        null=True,
        blank=True,
    )
    reporting_type = models.ForeignKey(
        MasterConfig,
        on_delete=models.CASCADE,
        related_name="emp_reporting_type",
        null=True,
        blank=True,
    )
    level = models.IntegerField(null=True, blank=True)
    reporting_to = models.ManyToManyField(
        PrimaryEmpInfo, related_name="reporting_to_primempinfo", null=True, blank=True
    )
